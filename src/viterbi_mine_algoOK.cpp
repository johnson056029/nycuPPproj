...
namespace StochHMM {
	...
	void trellis::simple_viterbi(model* h, sequences* sqs_x, sequences* sqs_y){
		//Initialize the table
		hmm = h;
		seqs_x = sqs_x;
		seqs_y = sqs_y;
		seq_x_size		= seqs_x->getLength();
		seq_y_size		= seqs_y->getLength();
		state_size		= hmm->state_size();
		exDef_defined	= seqs->exDefDefined();
	...
	void trellis::simple_viterbi(){
		...
		// traceback_table = new int_2D(seq_size,std::vector<int16_t> (state_size,-1));
                ...
                auto traceback_table= std::vector< std::vector<double>> (seq_x_size, std::vector<double> (seq_y_size));
                auto scoring_table = std::vector< std::vector<std::vector<double>> (seq_x_size+1, 
                        std::vector<std::vector<double> (seq_y_size+1, 
		...
		for(size_t iter_seq_x = 0; iter_seq_x < seq_x_size; ++iter_seq_x){
                        scoring_table[iter_seq_x][0][MMATCH] = -INFINITY;
                        scorinf_table[iter_seq_x][0][INST_X] = -INFINITY;
                        scoring_table[iter_seq_x][0][INST_Y] = -INFINITY;
		}
		for(size_t iter_seq_y = 0; iter_seq_y < seq_y_size; ++iter_seq_y){
                        scoring_table[0][iter_seq_y][MMATCH] = -INFINITY;
                        scorinf_table[0][iter_seq_y][INST_X] = -INFINITY;
                        scoring_table[0][iter_seq_y][INST_Y] = -INFINITY;
		}
